...
type AppView = 'dashboard' | 'entry' | 'manage' | 'summary' | 'bulk-edit';

// --- TYPE DEFINITIONS for BoxStatus ---
type BoxStatus = 'default' | 'updated' | 'stale';
...
const getInitialData = (): AppData => {
    ...
    return {
        ...
        customTraits: [] as TraitDefinition[],
    };
};


const getBoxStatus = (box: BoxData): BoxStatus => {
  if (!box.date) {
    return 'default';
  }
...
const getGermanStatus = (status: BoxStatus): string => {
    switch (status) {
...
const getBoxStyle = (box: BoxData): React.CSSProperties => {
...
const OptionGrid: React.FC<OptionGridProps> = ({ label, options, selectedValues = [], onToggle, multiSelect = false, labelAction }) => {
...
interface DataEntryScreenProps {
    box: BoxData;
    onSave: (id: number, data: Partial<Omit<BoxData, 'id'>>) => void;
...
const DataEntryScreen: React.FC<DataEntryScreenProps> = ({ box, onSave, onClear, onBack, varieties, sortings, fillLevels, customTraits }) => {
  const [formData, setFormData] = useState<Partial<Omit<BoxData, 'id'>>>({
  ...
  const handleVarietyToggle = (toggledVariety: string) => {
    setFormData(prev => {
  ...
  const handleFillLevelSelect = (value: string) => {
    setFormData(prev => ({ ...prev, fillLevel: value }));
  };
  
  const handleCustomTraitChange = (traitName: string, value: string) => {
    setFormData(prev => ({
        ...prev,
        customTraits: {
            ...(prev.customTraits || {}),
...
interface BulkEditScreenProps {
  selectedBoxIds: number[];
  onBulkSave: (data: Partial<Omit<BoxData, 'id'>>) => void;
  onBulkClear: () => void;
  onBack: () => void;
  varieties: string[];
  sortings: string[];
...
const BulkEditScreen: React.FC<BulkEditScreenProps> = (props) => {
    ...
    const handleSelect = (fieldName: keyof Pick<BoxData, 'sorting' | 'fillLevel'> | 'customTrait', value: string, traitName?: string) => {
    ...
    const handleSave = (e: React.FormEvent) => {
        ...
        const dataToSave: Partial<Omit<BoxData, 'id'>> = {};
...
interface EditableListProps {
  ...
  onRename: (oldItem: string, newItem: string) => void;
  onDelete: (itemToDelete: string) => void;
}

const EditableList: React.FC<EditableListProps> = ({ title, items, onAdd, onRename, onDelete }) => {
  ...
  const handleRenameClick = (item: string) => {
    setEditingItem(item);
  ...
  const handleRenameSave = (oldItem: string) => {
  ...
  const handleConfirmDelete = (item: string) => {
    onDelete(item);
    setDeletingItem(null);
  };
...
interface ManageTraitsScreenProps {
  varieties: string[];
  sortings: string[];
  fillLevels: string[];
  customTraits: TraitDefinition[];
  onBack: () => void;
  onAddVariety: (name: string) => void;
  onRenameVariety: (oldName: string, newName: string) => void;
  onDeleteVariety: (name: string) => void;
  onAddSorting: (name: string) => void;
  onRenameSorting: (oldName: string, newName: string) => void;
  onDeleteSorting: (name: string) => void;
  onAddFillLevel: (name: string) => void;
  onRenameFillLevel: (oldName: string, newName: string) => void;
  onDeleteFillLevel: (name: string) => void;
  onAddTrait: (name: string) => void;
  onRenameTrait: (oldName: string, newName: string) => void;
  onDeleteTrait: (name: string) => void;
  onAddTraitOption: (traitName: string, option: string) => void;
  onRenameTraitOption: (traitName: string, oldOption: string, newOption: string) => void;
  onDeleteTraitOption: (traitName: string, option: string) => void;
  onDownloadCsv: () => void;
}

const ManageTraitsScreen: React.FC<ManageTraitsScreenProps> = (props) => {
  const {
  ...
  const [deletingTraitName, setDeletingTraitName] = useState<string | null>(null);
  ...
  const handleRenameTraitClick = (name: string) => {
  ...
  const handleRenameTraitSave = (oldName: string) => {
  ...
  const handleDeleteTraitClick = (traitName: string) => {
    setDeletingTraitName(traitName);
    setEditingTraitName(null);
  };
  
  const handleConfirmDeleteTrait = (traitName: string) => {
...
const getFillLevelWeight = (fillLevel: string | undefined): number => {
    switch (fillLevel) {
        case '100%': return 1.0;
        case '75%': return 0.75;
        case '50%': return 0.5;
...
const SummaryScreen: React.FC<SummaryScreenProps> = ({ boxes, onBack }) => {
  const summaryData = useMemo(() => {
    type SortingDetail = {
      ...
      fillLevels: { [fillLevel: string]: number };
    };
    type VarietyDetail = {
      weightedTotal: number;
      sortings: { [sorting: string]: SortingDetail };
    };
    type Summary = {
      [variety: string]: VarietyDetail;
    };
...
const App = () => {
  ...
  const handleDownloadCsv = () => {
    ...
    const escapeCsvCell = (cellData: string | undefined | null) => {
  ...
  // --- BOX DATA HANDLERS ---
  const handleSaveBox = (id: number, data: Partial<Omit<BoxData, 'id'>>) => {
    if (!appData) return;
  ...
  const handleClearBox = (id: number) => {
    if (!appData) return;
    const newBoxes = appData.boxes.map(box =>
  ...
  const handleToggleBoxSelection = (id: number) => {
    setSelectedBoxIds(prev =>
      prev.includes(id) ? prev.filter(boxId => boxId !== id) : [...prev, id]
    );
  };

  const handleBulkSave = (dataToApply: Partial<Omit<BoxData, 'id'>>) => {
  ...
  const handleRenameVariety = (oldName: string, newName: string) => {
    if (!appData) return;
  ...
  const handleDeleteVariety = (name: string) => {
    if (!appData) return;
  ...
  // Sortings
  const handleAddSorting = (name: string) => { 
    if (!appData) return;
    const newSortings = [...(appData.sortings || []), name].sort();
    setAppData(prev => ({ ...prev!, sortings: newSortings }));
  };

  const handleRenameSorting = (oldName: string, newName: string) => {
    if (!appData) return;
  ...
  const handleDeleteSorting = (name: string) => {
    if (!appData) return;
    const newSortings = appData.sortings.filter(s => s !== name);
    const newBoxes = appData.boxes.map(box => {
        if (box.sorting === name) {
            const { sorting, ...restOfBox } = box as any; // Use 'as any' to bypass strict type checking for property deletion
            return restOfBox;
        }
        return box;
    });
    setAppData(prev => ({ ...prev!, sortings: newSortings, boxes: newBoxes as BoxData[] }));
  };


  // Fill Levels
  const handleAddFillLevel = (name: string) => {
      if (!appData) return;
      const newFillLevels = [...(appData.fillLevels || []), name].sort();
      setAppData(prev => ({ ...prev!, fillLevels: newFillLevels }));
  };

  const handleRenameFillLevel = (oldName: string, newName: string) => {
    if (!appData) return;
  ...
  const handleDeleteFillLevel = (name: string) => {
    if (!appData) return;
    const newFillLevels = appData.fillLevels.filter(f => f !== name);
    const newBoxes = appData.boxes.map(box => {
        if (box.fillLevel === name) {
            const { fillLevel, ...restOfBox } = box as any;
            return restOfBox;
        }
        return box;
    });
    setAppData(prev => ({ ...prev!, fillLevels: newFillLevels, boxes: newBoxes as BoxData[] }));
  };


  // Custom Trait Definitions
  const handleAddTrait = (name: string) => {
    if (!appData) return;
  ...
  const handleRenameTrait = (oldName: string, newName: string) => {
    if (!appData) return;
  ...
  const handleAddTraitOption = (traitName: string, option: string) => {
    if (!appData) return;
  ...
  const handleRenameTraitOption = (traitName: string, oldOption: string, newOption: string) => {
    if (!appData) return;
    const newTraits = appData.customTraits.map(trait => {
        if (trait.name === traitName) {
  ...
  const onDeleteTraitOption = (traitName: string, option: string) => {
    if (!appData) return;
    const newTraits = appData.customTraits.map(trait => {
        if (trait.name === traitName) {
